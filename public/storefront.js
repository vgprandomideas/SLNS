...
function checkout(productId, product) {
  ...
  document.querySelector("#store-form").addEventListener("submit", async (event) => { event.preventDefault(); const payload = Object.fromEntries(new FormData(event.currentTarget)); payload.productId = productId; payload.qty = Number(payload.qty); payload.marketingConsent = payload.marketingConsent === "true"; try { const response = await fetch("/api/storefront/orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || "Reservation failed"); let paymentMessage = "We’ll confirm delivery shortly."; if (payload.paymentMode === "Prepaid") { const paymentResponse = await fetch("/api/storefront/payment-intent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId: result.orderId, amount: result.amount, email: payload.email, idempotencyKey: `web-${result.orderId}` }) }); const payment = await paymentResponse.json(); if (paymentResponse.ok) paymentMessage = `Payment ${String(payment.paymentIntent.status).toLowerCase()} · we’ll confirm delivery shortly.`; } document.querySelector("#store-modal").remove(); showToast(`Reserved ${result.orderId}. ${paymentMessage}`); } catch (error) { showToast(error.message, true); } });
}
