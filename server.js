...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname.startsWith("/api/")) {
    ...
    if (url.pathname === "/api/storefront/payment-confirm") {
      const intent = store.paymentIntents.find((candidate) => candidate.id === payload.paymentIntentId); const order = intent && store.orders.find((candidate) => candidate.id === intent.orderId); const customer = order && store.customers.find((candidate) => candidate.id === order.customerId);
      if (!intent || !order || !customer || !payload.email || customer.email !== payload.email) return bad(res, "Payment intent or customer verification failed");
      if (intent.status === "Captured") return json(res, 200, { ok: true, orderId: order.id, status: order.status, paymentIntent: intent });
      let provider; try { provider = await callProvider("payment", { operation: "capture", paymentIntentId: intent.id, orderId: order.id, amount: intent.amount, method: payload.paymentMethod || "UPI", reference: payload.reference || null }); } catch (error) { return json(res, 502, { error: error.message }); }
      intent.status = "Captured"; intent.capturedAt = now(); intent.paymentMethod = payload.paymentMethod || "UPI"; intent.captureReference = payload.reference || provider.reference || null; order.status = "Confirmed";
      const receipt = { id: id("RCT").toUpperCase(), invoiceId: null, customer: order.customer, amount: intent.amount, mode: intent.paymentMethod, status: "Reconciled", receivedAt: now(), orderId: order.id }; store.receipts.unshift(receipt); postJournal(receipt.id, `Website payment · ${order.customer}`, `Bank - ${intent.paymentMethod}`, "Website orders", intent.amount, "Storefront"); enqueue("payment.captured", intent.id, "Internal notifications", { orderId: order.id, amount: intent.amount }); audit("CAPTURE", "PAYMENT", intent.id, `Captured ${intent.amount} INR for ${order.id}`); persist(); return json(res, 200, { ok: true, orderId: order.id, status: order.status, receipt, paymentIntent: intent });
    }
